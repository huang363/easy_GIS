...
t_transform_coordinates get_trans_coord() { return trans_coord; }
...
void drawtext_in(const t_bound_box& bbox, const std::string& text, bool centered) {
    drawtext(bbox.get_center(), text, bbox,  centered);
}

void drawtext_in(const t_bound_box& bbox, const std::string& text, float tolerance, bool centered) {
    drawtext(bbox.get_center(), text, bbox, tolerance,  centered);
}

void drawtext(
    const t_point& text_center, const std::string& text, const t_bound_box& bounds, float tolerance, bool centered) {
    ...
    drawtext(text_center, text, tolerance_bounds,  centered);
}

void drawtext(const t_point& text_center, const std::string& text, const t_bound_box& bounds, bool centered) {
...
//        printf("the center of the text \"%s\" isn't in its bounding box", text.c_str());
    ...
    min_bounds *= 2;
    drawtext(text_center, text, min_bounds.x, min_bounds.y,  centered);
}

void drawtext(const t_point& text_center, const std::string& text, float boundx, float boundy, bool centered) {
    drawtext(text_center.x, text_center.y, text, boundx, boundy,  centered);
}
 ...
 */
void drawtext(float xc, float yc, const std::string& str_text, float boundx, float boundy, bool centered) {
    ...
    float X11_xmagicoffset;//width * trans_xmult;
    float X11_ymagicoffset;
    if (centered){
         X11_xmagicoffset = 0; //width * trans_xmult;
         X11_ymagicoffset = 0;
    }
    else{
        X11_xmagicoffset = width * trans_xmult; //width * trans_xmult;
        X11_ymagicoffset = height * trans_ymult;
    }
#endif
